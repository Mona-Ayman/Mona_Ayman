...
if($_GET){
    if(isset($_GET['category'])){
        $categoryModel->setId($_GET['category']);
        if($categoryModel->getCategory()->num_rows == 1){
            $productModel->setcategory_id($_GET['category']);
            $productArry = $productModel->getProductsByCategory()->fetch_all(MYSQLI_ASSOC);
        }else{
            header('location:errors/404.php');die;
        }
    }
    elseif(isset($_GET['subcategory'])){
        $subcategoryModel->setId($_GET['subcategory']);
        if($subcategoryModel->getSubcategory()->num_rows == 1){
            $productModel->setSubcategory_id($_GET['subcategory']);
            $productArry = $productModel->getProductsBySubategory()->fetch_all(MYSQLI_ASSOC);
        }else{
            header('location:errors/404.php');die;
        }
    }
    elseif(isset($_GET['brand'])){
        ...
        if($brandModel->getBrand()->num_rows == 1){
            $productModel->setBrand_id($_GET['brand']);
            $productArry = $productModel->getProductsByBrand()->fetch_all(MYSQLI_ASSOC);
        }else{
            header('location:errors/404.php');die;
        }
    }else{
    ...
    $productArry = $productModel->getAllProducts()->fetch_all(MYSQLI_ASSOC);
            ...
            <div class="col-lg-9">
                ...
                <div class="grid-list-product-wrapper">
                    <div class="product-grid product-view pb-20">
                        <div class="row">
                            <?php foreach($productArry AS $product){ ?>
